...
class Program
{
    ...
    {
        if (args.Length == 0) {
            Console.WriteLine("Usage: AdobeTypeGen <in adobe-type>.xml <out adobe-type>.d.ts");
            return;
        }
        ...
        List<IExpr> classDefs = GenerateClassDefs(xmlDoc);
        List<IExpr> customTypes = GenerateCustomTypes();

        var result = new StringBuilder();

        result.AppendLine("/// <reference path=\"global.d.ts\" />");
        foreach (var cls in classDefs) {
            if (IgnoreTypes.IndexOf(cls.Name) >= 0) {
                continue;
            }
            result.AppendLine(cls.ToExpr());
        }

        foreach (var ct in customTypes) {
            result.AppendLine(ct.ToExpr());
        }

        File.WriteAllText(dtsPath, result.ToString());
    ...
    public static List<IExpr> GenerateCustomTypes()
    {
        var result = new List<IExpr>();
        result.Add(new ClassDef {
            Name = "ElementPlacement",
            Description = "The placement of an element in a container.",
            IsEnum = true,
            Props = {
                new PropertyDef {
                    Name = "INSIDE",
    ...
    public static List<IExpr> GenerateClassDefs(XElement xmlDoc)
    {
